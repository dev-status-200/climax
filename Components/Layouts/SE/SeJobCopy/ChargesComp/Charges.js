      ...
      return (<>
        ...
        <tr key={index} className='f table-row-center-singleLine'>
            ...
            <td style={{ padding: 3, minWidth: 50 }}> {/* PP?CC */}
                <SelectComp register={register} name={`chargeList.${index}.pp_cc`} control={control} width={60} font={13} disabled={permissions.admin?false:x.InvoiceId!=null?true:false}
                    options={[
                        { id: 'PP', name: 'PP' },
                        { id: 'CC', name: 'CC' }
                    ]}
            ...
            <td style={{ padding: 3 }}> {/* Size/Type */}
                <SelectSearchComp register={register} name={`chargeList.${index}.size_type`} control={control} width={100} font={13} disabled={permissions.admin?false:x.InvoiceId!=null?true:false}
                    options={[
                        { id: '40HC', name: '40HC' },
                        { id: '20HC', name: '20HC' }
                    ]}
            ...
            <td style={{ padding: 3 }}> {/* Amount */}
                <InputNumComp register={register} name={`chargeList.${index}.amount`} control={control} label='' width={20} disabled={permissions.admin?false:x.InvoiceId!=null?true:false} />
            </td>
            <td style={{ padding: 3 }}>  {/* Discount */}
                <InputNumComp register={register} name={`chargeList.${index}.discount`} control={control} width={30} font={13} disabled={permissions.admin?false:x.InvoiceId!=null?true:false} />
            </td>
            <td style={{ textAlign: 'center' }}> {/* Tax Apply */}
                <input type="checkbox" {...register(`chargeList.${index}.tax_apply`)} style={{ cursor: 'pointer' }} disabled={permissions.admin?false:x.InvoiceId!=null?true:false} />
            </td>
            <td>{x.tax_amount}</td> {/* Tax Amount */}
            <td>{x.net_amount}</td>
            <td style={{ padding: 3 }}> {/* Ex. Rate */}
                <InputNumComp register={register} name={`chargeList.${index}.ex_rate`} control={control} label='' width={10} disabled={permissions.admin?false:x.InvoiceId!=null?true:false}/>
            </td>
